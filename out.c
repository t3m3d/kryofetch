...
char* getDiskInfo() {
    char* out = kr_str("");
    char* drives = GetLogicalDrives();
    char* letter = kr_str("65");
    while (kr_truthy(kr_lte(kr_toint(letter), kr_str("90")))) {
        char* bit = kr_lt(kr_str("1"), kr_str(""));
        kr_sub(kr_toint(letter), kr_toint(kr_str("65")));
        if (kr_truthy(drives)) {
            kr_str("");
            kr_str("0");
            kr_str("");
            letter = kr_plus(kr_plus(kr_toint(letter), kr_str("1")), kr_str(""));
            continue;
        }
        char* root = kr_plus(kr_fromcharcode(letter), kr_str(":\\"));
        char* freeBytes = ULARGE;
        ({ char* _sl = kr_structnew(); _sl; });
        char* totalBytes = ULARGE;
        ({ char* _sl = kr_structnew(); _sl; });
        char* totalFreeBytes = ULARGE;
        ({ char* _sl = kr_structnew(); _sl; });
        if (kr_truthy(kr_eq(GetDiskFreeSpaceExA(root, freeBytes, totalBytes, totalFreeBytes), kr_str("0")))) {
            char* line = kr_plus(root, kr_str("  (access denied)\n"));
            out = kr_plus(out, line);
            letter = kr_plus(kr_plus(kr_toint(letter), kr_str("1")), kr_str(""));
            continue;
        }
        char* totalGB = kr_plus(kr_div(kr_getfield(totalBytes, "QuadPart"), kr_mul(kr_mul(kr_str("1024"), kr_str("1024")), kr_str("1024"))), kr_str(""));
        char* freeGB = kr_plus(kr_div(kr_getfield(totalFreeBytes, "QuadPart"), kr_mul(kr_mul(kr_str("1024"), kr_str("1024")), kr_str("1024"))), kr_str(""));
        char* usedGB = kr_plus(kr_sub(kr_toint(totalGB), kr_toint(freeGB)), kr_str(""));
        char* percent = kr_str("0");
        if (kr_truthy(kr_gt(kr_toint(totalGB), kr_str("0")))) {
            percent = kr_plus(kr_div(kr_mul(kr_toint(usedGB), kr_str("100")), kr_toint(totalGB)), kr_str(""));
        }
        char* width = kr_str("20");
        char* used = kr_plus(kr_div(kr_mul(kr_toint(percent), kr_toint(width)), kr_str("100")), kr_str(""));
        char* k_free = kr_plus(kr_sub(kr_toint(width), kr_toint(used)), kr_str(""));
        char* bar = kr_str("");
        char* i = kr_str("0");
        while (kr_truthy(kr_lt(kr_toint(i), kr_toint(used)))) {
            bar = kr_plus(bar, kr_str("▓"));
            i = kr_plus(kr_plus(kr_toint(i), kr_str("1")), kr_str(""));
        }
        i = kr_str("0");
        while (kr_truthy(kr_lt(kr_toint(i), kr_toint(k_free)))) {
            bar = kr_plus(bar, kr_str("█"));
            i = kr_plus(kr_plus(kr_toint(i), kr_str("1")), kr_str(""));
        }
        char* line = kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(root, kr_str("  ")), usedGB), kr_str("GB / ")), totalGB), kr_str("GB  ")), percent), kr_str("%  [")), bar), kr_str("]\n"));
        out = kr_plus(out, line);
        letter = kr_plus(kr_plus(kr_toint(letter), kr_str("1")), kr_str(""));
    }
    return out;
...
char* get(char* info) {
    char* total = kr_str("0");
    char* k_free = kr_str("0");
    char* ms = ({ char* _sl = kr_structnew(); _sl; });
    kr_setfield(ms, "dwLength", k_sizeof(MEMORYSTATUSEX));
    if (kr_truthy(GlobalMemoryStatusEx(ms))) {
        char* t = kr_div(kr_getfield(ms, "ullTotalPhys"), kr_mul(kr_str("1024"), kr_str("1024")));
        char* f = kr_div(kr_getfield(ms, "ullAvailPhys"), kr_mul(kr_str("1024"), kr_str("1024")));
        total = kr_plus(t, kr_str(""));
...
char* make(char* bar, char* usedGB, char* totalGB) {
    if (kr_truthy(kr_lte(totalGB, kr_str("0.0")))) {
        return kr_str("[--------------------] 0.0 / 0.0 GB");
    }
    char* pct = kr_mul(kr_div(usedGB, totalGB), kr_str("100.0"));
    char* blocks = kr_toint(kr_div(pct, kr_str("5.0")));
    char* bar = kr_str("");
    char* i = kr_str("0");
    while (kr_truthy(kr_lt(kr_toint(i), blocks))) {
        bar = kr_plus(bar, kr_str("#"));
        i = kr_plus(kr_plus(kr_toint(i), kr_str("1")), kr_str(""));
    }
    while (kr_truthy(kr_lt(kr_toint(i), kr_str("20")))) {
        bar = kr_plus(bar, kr_str("-"));
        i = kr_plus(kr_plus(kr_toint(i), kr_str("1")), kr_str(""));
    }
    return kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_plus(kr_str("["), bar), kr_str("] ")), usedGB), kr_str(" / ")), totalGB), kr_str(" GB"));
}

char* print(char* ansi, char* s, char* totalWidth) {
    kr_print(s);
    char* pad = kr_sub(kr_toint(totalWidth), kr_len(s));
    if (kr_truthy(kr_lt(pad, kr_str("0")))) {
        pad = kr_str("0");
    }
    char* i = kr_str("0");
    ...
    return kr_str("");
    if (kr_truthy(kr_not(GetComputerNameA(buf, len)))) {
        return kr_str("Unknown");
    }
    return buf;
}

char* get(char* size) {
    char* cols = kr_plus(GetSystemMetrics(kr_str("0")), kr_str(""));
    char* rows = kr_plus(GetSystemMetrics(kr_str("1")), kr_str(""));
    return kr_plus(kr_plus(cols, kr_str(",")), rows);
}
